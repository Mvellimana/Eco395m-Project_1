...
Have you thought about using web scraping for your potential dish scraping business? If yes, this report would give you an applicable method to follow, which would pay off in the future. The goal of the project is to provide an insight to potential investors in food & beverage sector. The idea here is to scrape Yelp restaurants data, clean it, then analyze it by finding out which cuisine has a market niche, and where it locates in Austin?
...
### Aggregate & Strip
The Yelp scraped dataset included 124 different categories for restaurants. That was a lot! So, these were aggregated into 10 broader groups to analyze which category would be best to invest in. Then, we used coding magic to separate (strip) Zipcodes from address data, and `latitude` and `longitude` values from `cooridinates` information. In addition, `transaction types` were separated to indivdual columns with binary values for each unique type.

### Standarized the scales
Also, ratings and review counts were standardized by z-scoring to have them within the same scale for the analytical part. Finally, we converted the price level from `$` to numbers from 1-4 for averaging analysis.
...
Austin restaurants seem to be popular among people with a mean rating of 4.1 evaluated from 739 restaurants. Nearly 84% of the restaurants offer delivery. This seems coherent with the busy lifestyle of Austinites. 
...
![Best Restaurants Category](/artifacts/figures/best_restaurant.png)
...
![Market Niche for Food Trucks by Zip Code](/artifacts/figures/market_niech.png)
...
While Yelp has given access to plethora of things, the data could be cleaner. We found 124 food categories across 739 restaurants which hindered any deep analysis. The dataset was also not devoid of missing values. 

We dont have the detailed price and sales volume for every restaurant which limit our chances of doing more granular analysis.
